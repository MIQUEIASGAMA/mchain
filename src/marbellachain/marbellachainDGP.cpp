...
void MarbellaChainDGP::initDataEIP158(){
...
bool MarbellaChainDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData){
...
dev::eth::EVMSchedule MarbellaChainDGP::getGasSchedule(unsigned int blockHeight){
    clear();
...
uint64_t MarbellaChainDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t MarbellaChainDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t MarbellaChainDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t MarbellaChainDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool MarbellaChainDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void MarbellaChainDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void MarbellaChainDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void MarbellaChainDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void MarbellaChainDGP::createParamsInstance(){
...
dev::Address MarbellaChainDGP::getAddressForBlock(unsigned int blockHeight){
...
void MarbellaChainDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void MarbellaChainDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void MarbellaChainDGP::parseStorageOneUint64(uint64_t& value){
...
void MarbellaChainDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = uint64_t(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule MarbellaChainDGP::createEVMSchedule(){
    dev::eth::EVMSchedule schedule = dev::eth::EIP158Schedule;
