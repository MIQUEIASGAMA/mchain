...
#include <marbellachain/marbellachaintransaction.h>
...
class MarbellaChainState : public dev::eth::State {
    
public:

    MarbellaChainState();

    MarbellaChainState(dev::u256 const& _accountStartNonce, dev::OverlayDB const& _db, const std::string& _path, dev::eth::BaseState _bs = dev::eth::BaseState::PreExisting);

    ResultExecute execute(dev::eth::EnvInfo const& _envInfo, dev::eth::SealEngineFace const& _sealEngine, MarbellaChainTransaction const& _t, dev::eth::Permanence _p = dev::eth::Permanence::Committed, dev::eth::OnOpFunc const& _onOp = OnOpFunc());
...
private:
    ...
    dev::Address createMarbellaChainAddress(dev::h256 hashTx, uint32_t voutNumber);
...
struct TemporaryState{
    std::unique_ptr<MarbellaChainState>& globalStateRef;
    dev::h256 oldHashStateRoot;
    dev::h256 oldHashUTXORoot;

    TemporaryState(std::unique_ptr<MarbellaChainState>& _globalStateRef) : 
...
public:
    ...
    CondensingTX(MarbellaChainState* _state, const std::vector<TransferInfo>& _transfers, const MarbellaChainTransaction& _transaction, std::set<dev::Address> _deleteAddresses = std::set<dev::Address>()) : transfers(_transfers), deleteAddresses(_deleteAddresses), transaction(_transaction), state(_state){}
...
private:
    ...
    const MarbellaChainTransaction& transaction;

    MarbellaChainState* state;
