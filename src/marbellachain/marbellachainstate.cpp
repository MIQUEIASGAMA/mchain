...
MarbellaChainState::MarbellaChainState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = MarbellaChainState::openDB(_path + "/marbellachainDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

MarbellaChainState::MarbellaChainState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute MarbellaChainState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, MarbellaChainTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createMarbellaChainAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
    ...
    try{
        ...
        if (_p == Permanence::Reverted){
            ...
            marbellachain::commit(cacheUTXO, stateUTXO, m_cache);
            cacheUTXO.clear();
...
std::unordered_map<dev::Address, Vin> MarbellaChainState::vins() const // temp
{
...
void MarbellaChainState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* MarbellaChainState::vin(dev::Address const& _a) const
{
    return const_cast<MarbellaChainState*>(this)->vin(_a);
}

Vin* MarbellaChainState::vin(dev::Address const& _addr)
{
...
// void MarbellaChainState::commit(CommitBehaviour _commitBehaviour)
// {
...
void MarbellaChainState::kill(dev::Address _addr)
{
...
void MarbellaChainState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
dev::Address MarbellaChainState::createMarbellaChainAddress(dev::h256 hashTx, uint32_t voutNumber){
...
void MarbellaChainState::deleteAccounts(std::set<dev::Address>& addrs){
...
void MarbellaChainState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void MarbellaChainState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
