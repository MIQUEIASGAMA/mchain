...
#define paternVersion "mchain-([0-9]+\\.)?([0-9]+\\.)?([0-9]+)-"

MchainVersionChecker::MchainVersionChecker(QObject *parent) : QObject(parent)
{
    currentVersion = Version(CLIENT_VERSION_MAJOR, CLIENT_VERSION_MINOR, CLIENT_VERSION_REVISION);
}

MchainVersionChecker::~MchainVersionChecker()
{

}

bool MchainVersionChecker::newVersionAvailable()
{
    Version maxReleaseVersion = getMaxReleaseVersion();
    return maxReleaseVersion > currentVersion;
}

QList<Version> MchainVersionChecker::getVersions()
{
    QNetworkAccessManager manager;
    QNetworkReply *response = manager.get(QNetworkRequest(QUrl(MCHAIN_RELEASES)));
    QEventLoop event;
...
Version MchainVersionChecker::getMaxReleaseVersion()
{
