...
#define paternVersion "marbellachain-([0-9]+\\.)?([0-9]+\\.)?([0-9]+)-"

MarbellaChainVersionChecker::MarbellaChainVersionChecker(QObject *parent) : QObject(parent)
{
    currentVersion = Version(CLIENT_VERSION_MAJOR, CLIENT_VERSION_MINOR, CLIENT_VERSION_REVISION);
}

MarbellaChainVersionChecker::~MarbellaChainVersionChecker()
{

}

bool MarbellaChainVersionChecker::newVersionAvailable()
{
    Version maxReleaseVersion = getMaxReleaseVersion();
    return maxReleaseVersion > currentVersion;
}

QList<Version> MarbellaChainVersionChecker::getVersions()
{
    QNetworkAccessManager manager;
    QNetworkReply *response = manager.get(QNetworkRequest(QUrl(MARBELLACHAIN_RELEASES)));
    QEventLoop event;
...
Version MarbellaChainVersionChecker::getMaxReleaseVersion()
{
