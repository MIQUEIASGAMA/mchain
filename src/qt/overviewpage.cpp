...
public:
                      ...
                      const QModelIndex &index ) const
    {
        ...
        QColor txColor = index.row() % 2 ? QColor("#ffffff") : QColor("#efefef");
        painter->fillRect(mainRect, txColor);

        QPen pen;
        pen.setWidth(2);
        pen.setColor(QColor("#0099bf"));
        painter->setPen(pen);
...
public:
               ...
               const QModelIndex &index) const
    {
        ...
        QString tokenBalance = index.data(TokenItemModel::BalanceRole).toString().remove(QRegExp("\\s+")).remove(QRegExp("\\.?0*$"));

        QRect mainRect = option.rect;
        mainRect.setWidth(option.rect.width());

        painter->fillRect(mainRect, QColor("#ffffff"));

        QRect hLineRect(mainRect.left(), mainRect.bottom(), mainRect.width(), 1);
        painter->fillRect(hLineRect, QColor("#efefef"));
    ...
    tkndelegate(new TknViewDelegate(platformStyle, this))
{
    ...
    SetObjectStyleSheet(ui->buttonAddToken, StyleSheetNames::ButtonWhite);
    SetObjectStyleSheet(ui->showMoreButton, StyleSheetNames::ButtonWhite);
