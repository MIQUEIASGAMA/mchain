    ...
    m_results(1)
{
    ...
    // Set stylesheet
    SetObjectStyleSheet(ui->pushButtonClearAll, StyleSheetNames::ButtonWhite);
...
void CreateContract::setLinkLabels()
{
    ...
    ui->labelSolidity->setText("<a href=\"https://remix.ethereum.org/\" style=\"font-weight:bold;\">"+tr("Solidity compiler")+"</a>");

    ui->labelToken->setOpenExternalLinks(true);
    ui->labelToken->setText("<a href=\"https://ethereum.org/token#the-code\" style=\"margin-left:10px;font-weight:bold;\">"+tr("Token template")+"</a>");
}
...
void CreateContract::on_numBlocksChanged()
{
    if(m_clientModel)
    {
        ...
        ui->labelGasPrice->setToolTip(tr("Gas price: MAR price per gas unit. Default = %1, Min = %2").arg(QString::fromStdString(FormatMoney(DEFAULT_GAS_PRICE))).arg(QString::fromStdString(FormatMoney(minGasPrice))));
