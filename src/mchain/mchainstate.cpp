...
MchainState::MchainState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = MchainState::openDB(_path + "/mchainDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

MchainState::MchainState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute MchainState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, MchainTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createMchainAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
    ...
    try{
        ...
        if (_p == Permanence::Reverted){
            ...
            mchain::commit(cacheUTXO, stateUTXO, m_cache);
            cacheUTXO.clear();
...
std::unordered_map<dev::Address, Vin> MchainState::vins() const // temp
{
...
void MchainState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* MchainState::vin(dev::Address const& _a) const
{
    return const_cast<MchainState*>(this)->vin(_a);
}

Vin* MchainState::vin(dev::Address const& _addr)
{
...
// void MchainState::commit(CommitBehaviour _commitBehaviour)
// {
...
void MchainState::kill(dev::Address _addr)
{
...
void MchainState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
dev::Address MchainState::createMchainAddress(dev::h256 hashTx, uint32_t voutNumber){
...
void MchainState::deleteAccounts(std::set<dev::Address>& addrs){
...
void MchainState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void MchainState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
