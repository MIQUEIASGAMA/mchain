...
bool MchainDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData){
...
dev::eth::EVMSchedule MchainDGP::getGasSchedule(unsigned int blockHeight){
    clear();
...
uint64_t MchainDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t MchainDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t MchainDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t MchainDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool MchainDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void MchainDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void MchainDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void MchainDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void MchainDGP::createParamsInstance(){
...
dev::Address MchainDGP::getAddressForBlock(unsigned int blockHeight){
...
void MchainDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void MchainDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void MchainDGP::parseStorageOneUint64(uint64_t& value){
...
void MchainDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = uint64_t(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule MchainDGP::createEVMSchedule(){
    dev::eth::EVMSchedule schedule = dev::eth::EIP158Schedule;
