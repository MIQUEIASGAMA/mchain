...
#include <mchain/mchaintransaction.h>
...
class MchainState : public dev::eth::State {
    
public:

    MchainState();

    MchainState(dev::u256 const& _accountStartNonce, dev::OverlayDB const& _db, const std::string& _path, dev::eth::BaseState _bs = dev::eth::BaseState::PreExisting);

    ResultExecute execute(dev::eth::EnvInfo const& _envInfo, dev::eth::SealEngineFace const& _sealEngine, MchainTransaction const& _t, dev::eth::Permanence _p = dev::eth::Permanence::Committed, dev::eth::OnOpFunc const& _onOp = OnOpFunc());
...
struct TemporaryState{
    std::unique_ptr<MchainState>& globalStateRef;
    dev::h256 oldHashStateRoot;
    dev::h256 oldHashUTXORoot;

    TemporaryState(std::unique_ptr<MchainState>& _globalStateRef) : 
...
public:
    ...
    CondensingTX(MchainState* _state, const std::vector<TransferInfo>& _transfers, const MchainTransaction& _transaction, std::set<dev::Address> _deleteAddresses = std::set<dev::Address>()) : transfers(_transfers), deleteAddresses(_deleteAddresses), transaction(_transaction), state(_state){}
...
private:
    ...
    const MchainTransaction& transaction;
