...
void checkResult(bool isCreation, std::vector<MchainTransaction> results, uint256 hash){
...
void runTest(bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    MchainTxConverter converter(transaction, NULL);
    ExtractMchainTX mchainTx;
    BOOST_CHECK(converter.extractionMchainTransactions(mchainTx));
    std::vector<MchainTransaction> result = mchainTx.first;
    if(script2 == CScript()){
...
BOOST_FIXTURE_TEST_SUITE(mchaintxconverter_tests, TestingSetup)
