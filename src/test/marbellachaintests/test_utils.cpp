...
void initState(){
    ...
    const std::string dirMarbellaChain = pathTemp.string();
    const dev::h256 hashDB(dev::sha3(dev::rlp("")));
    globalState = std::unique_ptr<MarbellaChainState>(new MarbellaChainState(dev::u256(0), MarbellaChainState::openDB(dirMarbellaChain, hashDB, dev::WithExisting::Trust), dirMarbellaChain + "/marbellachainDB", dev::eth::BaseState::Empty));
...
MarbellaChainTransaction createMarbellaChainTransaction(valtype data, dev::u256 value, dev::u256 gasLimit, dev::u256 gasPrice, dev::h256 hashTransaction, dev::Address recipient, int32_t nvout){
    MarbellaChainTransaction txEth;
    if(recipient == dev::Address()){
        txEth = MarbellaChainTransaction(value, gasPrice, gasLimit, data, dev::u256(0));
    } else {
        txEth = MarbellaChainTransaction(value, gasPrice, gasLimit, recipient, data, dev::u256(0));
    }
...
std::pair<std::vector<ResultExecute>, ByteCodeExecResult> executeBC(std::vector<MarbellaChainTransaction> txs){
