...
void checkResult(bool isCreation, std::vector<MarbellaChainTransaction> results, uint256 hash){
...
void runTest(bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    MarbellaChainTxConverter converter(transaction, NULL);
    ExtractMarbellaChainTX marbellachainTx;
    BOOST_CHECK(converter.extractionMarbellaChainTransactions(marbellachainTx));
    std::vector<MarbellaChainTransaction> result = marbellachainTx.first;
    if(script2 == CScript()){
...
BOOST_FIXTURE_TEST_SUITE(marbellachaintxconverter_tests, TestingSetup)
