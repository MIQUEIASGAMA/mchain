...
*Setup instructions for a Gitian build of Mchain Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Mchain
...
- [Building Mchain Core](#building-mchain-core)
...
Clone the git repositories for mchain and Gitian.
...
```bash
sudo cp /home/debian/mchain/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the mchain repository under 'Fetch and create inputs' to install sources which require
...
To build Mchain Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the mchain repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/mchain/.git/
...
For example:
```bash
URL=https://github.com/laanwj/mchain.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit mchain=${COMMIT} --url mchain=${URL} ../mchain/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit mchain=${COMMIT} --url mchain=${URL} ../mchain/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit mchain=${COMMIT} --url mchain=${URL} ../mchain/contrib/gitian-descriptors/gitian-osx.yml
# if wanting to use a different version of eth-cpp-mchain:
./bin/gbuild --commit mchain=${COMMIT},cpp-eth-mchain=${ETHCOMMIT} --url mchain=${URL},cpp-eth-mchain=${ETHURL} ../mchain/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the mchain git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../mchain/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/mchain
SIGPATH=/some/root/path/mchain-detached-sigs

./bin/gbuild --url mchain=${BTCPATH},signature=${SIGPATH} ../mchain/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/mchain-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/mchain-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/mchain-osx-build.assert
```
...
[mchain/gitian.sigs](https://github.com/mchain/gitian.sigs/) repository, or if that's not possible create a pull
request. You can also mail the files to Jordan Earls (earlz@mchain.network) and he will commit them.