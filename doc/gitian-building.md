...
*Setup instructions for a Gitian build of MarbellaChain Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the MarbellaChain
...
- [Building MarbellaChain Core](#building-marbellachain-core)
...
Clone the git repositories for marbellachain and Gitian.
...
```bash
sudo cp /home/debian/marbellachain/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the marbellachain repository under 'Fetch and create inputs' to install sources which require
...
To build MarbellaChain Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the marbellachain repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/marbellachain/.git/
...
For example:
```bash
URL=https://github.com/laanwj/marbellachain.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit marbellachain=${COMMIT} --url marbellachain=${URL} ../marbellachain/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit marbellachain=${COMMIT} --url marbellachain=${URL} ../marbellachain/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit marbellachain=${COMMIT} --url marbellachain=${URL} ../marbellachain/contrib/gitian-descriptors/gitian-osx.yml
# if wanting to use a different version of eth-cpp-marbellachain:
./bin/gbuild --commit marbellachain=${COMMIT},cpp-eth-marbellachain=${ETHCOMMIT} --url marbellachain=${URL},cpp-eth-marbellachain=${ETHURL} ../marbellachain/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the marbellachain git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../marbellachain/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/marbellachain
SIGPATH=/some/root/path/marbellachain-detached-sigs

./bin/gbuild --url marbellachain=${BTCPATH},signature=${SIGPATH} ../marbellachain/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/marbellachain-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/marbellachain-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/marbellachain-osx-build.assert
```
...
[marbellachain/gitian.sigs](https://github.com/marbellachain/gitian.sigs/) repository, or if that's not possible create a pull
request. You can also mail the files to Jordan Earls (earlz@marbellachain.org) and he will commit them.